...
/*   Updated: 2024/10/13 17:04:42 by hakarape         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print(t_philo *d_philo, char *str)
{
	...
	{
		if (time >= 0 && is_sim_end(d_philo) != 1)
		{
...
void	clear(t_data *data)
{
	...
	while (i < data->num_of_philo)
	{
		...
		pthread_mutex_destroy(&(data->d_philo[i].mut_stop_time));
		pthread_mutex_destroy(&(data->d_philo[i].mut_last_eat));
		i++;
	}
	free(data->d_philo);
	free(data->forks);
